...
with st.sidebar:
    ...
    try:
        ...
        # bi.quality_trend doesn't exist until the data_quality_full_report
        # asset check has run at least once. Don't let this crash the page —
        # an uncaught exception here renders Streamlit's default light-themed
        # error box, which is what was showing up as a stray white panel.
        audit_df = pd.DataFrame()
        table_missing = True

    all_audits = audit_df.iloc[:, 0].tolist() if not audit_df.empty else []
    selected_audits = st.multiselect(
        "Audits", all_audits, default=all_audits, key="qt_audits", disabled=table_missing
    )

if table_missing:
    st.info(
        "No quality trend data found yet. Run the **data_quality_full_report** "
        "asset check first — it writes to `bi.quality_trend`."
    )
    st.stop()
...
try:
    ...
    st.error(f"Couldn't load quality trend data: {e}")
    st.stop()
